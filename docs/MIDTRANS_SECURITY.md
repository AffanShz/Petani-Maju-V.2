...
|----------|------------|--------|
| **Server Key** | Supabase Secrets | ✅ Terlindungi |
| **Aplikasi Klien** | Kode Flutter | ❌ Bisa di-decompile |
| **Kredensial** | `.gitignore` | ✅ Terlindungi |
...
**⚠️ Catatan Keamanan:** Jangan pernah commit file ini ke version control.
...
|--------|---------|--------|
| **401 Unauthorized** | Server Key tidak valid/hilang | Verifikasi key di Supabase Secrets → Redeploy function |
| **500 Server Error** | Env variable tidak ter-load | Periksa `MIDTRANS_SERVER_KEY` ada → Redeploy |
...
- ✅ Simpan `MIDTRANS_SERVER_KEY` hanya di Supabase Secrets
- ✅ Jangan hardcode kredensial di kode
- ✅ Tambahkan `secrets.json` ke `.gitignore`
- ✅ Regenerate keys jika ter-expose
- ✅ Gunakan proyek Supabase terpisah untuk infrastruktur pembayaran
- ✅ Log semua percobaan transaksi untuk audit trail
- ❌ Jangan pass Server Key melalui environment variables di git
- ❌ Jangan simpan keys di kode aplikasi atau dokumentasi
