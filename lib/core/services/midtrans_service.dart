...
class MidtransService {
  ...
  /// Membuat transaksi Snap di Midtrans via Supabase Edge Function
  Future<MidtransTransactionResult> createTransaction({
    ...
    try {
      ...
      if (supabaseUrl.isEmpty || supabaseAnonKey.isEmpty) {
        throw Exception('Supabase Edge credentials not configured');
      }
      ...
      debugPrint('[MidtransService] Calling Edge Function: $functionUrl');
      ...
      final response = await http.post(
        ...
        body: jsonEncode({
          ...
          'amount': amount,
          'planName': planName,
          'customerName': customerName.isEmpty ? 'Petani Maju' : customerName,
          'customerEmail': customerEmail.isEmpty ? 'petani@petanimaju.id' : customerEmail,
          'paymentMethod': paymentMethod,
        }),
      );

      debugPrint(
          '[MidtransService] Response status: ${response.statusCode}');
      ...
      // Fallback ke simulator jika ada error
      debugPrint(
      ...
      debugPrint(
          '[MidtransService] Exception saat menghubungi Edge Function: $e');
      return MidtransTransactionResult(
        orderId: generatedOrderId,
        redirectUrl: _simulatorUrlFor(paymentMethod),
        isLiveSandbox: false,
  ...
  String _simulatorUrlFor(String type) {
    switch (type) {
      ...
      case 'va':
        // Generic VA ΓÇö arahkan ke BNI sebagai default
        return 'https://simulator.sandbox.midtrans.com/bni/va/index';
      case 'qris':
      default:
